...
<body>
    ...
    <script>
        // ========== CONFIGURACIÓN CON LAS URLs PROPORCIONADAS ==========
        ...
        const APPS_SCRIPT_URL_ENVIAR_DATOS = 'https://script.google.com/macros/s/AKfycbyBG2WKaVVcMp3YPou8drWNmpYA-5ghDhRqkiLGllM2wjZ1lJZXsV3H7pFNPxDlSnfl6Q/exec';
        ...
        // ========== FUNCIONES CORREGIDAS PARA VARIABLES DE PRODUCTIVIDAD ==========
        ...
        // ========== FUNCIÓN CORREGIDA PARA GENERAR CAMPOS DETALLADOS ==========
                        ...
                        camposHTML += `
                            <div class="form-group">
                                <label for="${campoCompletoId}" ${configCampo.requerido ? 'class="required"' : ''}>
                                    ${configCampo.label}: <!-- 🔥 Usar label exacto para mapeo -->
                                </label>
                                <div class="variable-input-container">
                                    <input type="number" 
                                           id="${campoCompletoId}" 
                                           ...
                                           data-variable="${variableSeleccionada}"
                                           data-campo="${configCampo.label}"> <!-- 🔥 Guardar label para mapeo -->
        ...
        // ========== FUNCIÓN CORREGIDA - MAPEO EXACTO CON ENCABEZADOS DEL FORMULARIO ==========
        function diagnosticarMapeoVariables() {
            console.log('🔍 INICIANDO DIAGNÓSTICO DE MAPEO CORREGIDO');
            ...
            // INICIALIZAR TODAS LAS VARIABLES EN 0 O CADENA VACÍA
            const todasVariables = [
                ...
                "Municiones (Und)", "Niños, Niñas y Adolescentes Rescatados", "Nitrato de Amonio (Kg)",
                ...
                "Valor de los Bienes Inmuebles Incautados", "Vehículos", "Victimas Liberadas", "Víctimas Rescatadas", "Varios"
            ];

            // Inicializar todas las variables con sus nombres EXACTOS del formulario
            todasVariables.forEach(variable => {
                // Para campos de texto, usar cadena vacía, para numéricos usar "0"
            ...
            // 🔥 CORRECCIÓN PRINCIPAL: OBTENER VALOR REAL DE LA VARIABLE SELECCIONADA
            if (variableSeleccionada && variablesProductividad[variableSeleccionada]) {
                const configVariable = variablesProductividad[variableSeleccionada];
                console.log(`🎯 Procesando variable seleccionada: ${variableSeleccionada}`, configVariable);
                ...
                if (configVariable.tipo === 'numero' || configVariable.tipo === 'texto') {
                    ...
                    if (inputElement) {
                        // 🔥 OBTENER EL VALOR REAL DEL INPUT
                        const valorReal = inputElement.value.trim();
                        tablaVariables[variableSeleccionada] = valorReal || 
                            (configVariable.tipo === 'numero' ? '0' : '');
                        console.log(`  ✅ ${variableSeleccionada} = "${valorReal}" → "${tablaVariables[variableSeleccionada]}"`);
                    } else {
                        console.log(`  ❌ No se encontró el elemento: ${fieldId}`);
                    }
                } 
                else if (configVariable.tipo.startsWith('detalle_') && configVariable.campos) {
                    console.log(`  📊 Variable detallada con ${Object.keys(configVariable.campos).length} campos`);
                    
                    // Para variables detalladas, actualizar todos los subcampos
                    Object.entries(configVariable.campos).forEach(([campoId, configCampo]) => {
                        const campoCompletoId = `${variableSeleccionada.replace(/[^a-zA-Z0-9]/g, '_')}_${campoId}`;
                        const inputElement = document.getElementById(campoCompletoId);
                        ...
                        if (inputElement) {
                            const valorReal = inputElement.value.trim();
                            const claveSubcampo = configCampo.label; // Usar el label exacto
                            
                            tablaVariables[claveSubcampo] = valorReal || '0';
                            console.log(`  ✅ ${claveSubcampo} = "${valorReal}"`);
                        } else {
                            console.log(`  ❌ No se encontró subcampo: ${campoCompletoId}`);
                        }
                    });
                }
            } else {
                console.log('⚠️  No hay variable seleccionada o configuración no encontrada');
            }

            // DEBUG: Mostrar valores específicos
            console.log('🔍 VALORES FINALES A ENVIAR:');
            console.log('  Variable seleccionada:', variableSeleccionada);
            console.log('  Valor de variable seleccionada:', tablaVariables[variableSeleccionada]);
            console.log('  "Aeronaves (Und)":', tablaVariables['Aeronaves (Und)']);
            console.log('  "Allanamientos":', tablaVariables['Allanamientos']);
            
            console.log('📊 TABLA DE VARIABLES COMPLETA:', tablaVariables);
            
            return tablaVariables;
        }

        // ========== FUNCIÓN DE VERIFICACIÓN PARA DEPURACIÓN ==========
        function verificarEnvioDatos() {
            const datosCompletos = recopilarDatosCompletos();
            
            console.log('🔍 VERIFICACIÓN PRE-ENVÍO:');
            console.log('1. Variable seleccionada:', datosCompletos.formulario.variablesProductividad);
            console.log('2. Valor de variable seleccionada en tabla:', 
                        datosCompletos.tablaVariables[datosCompletos.formulario.variablesProductividad]);
            console.log('3. Todos los valores en tablaVariables:');
            
            Object.entries(datosCompletos.tablaVariables).forEach(([key, value]) => {
                if (value !== '0' && value !== '') {
        ...
        // ========== FUNCIÓN ACTUALIZADA PARA RECOPILAR TODOS LOS DATOS ==========
        function recopilarDatosCompletos() {
            ...
            return {
                formulario: {
                    ...
                    variablesProductividad: document.getElementById('variablesProductividad').value
                },
                tablaVariables: tablaVariables, // 🔥 NUEVO: Tabla completa de todas las variables
                detenidos: recopilarDatosDetenidos(),
                totalDetenidos: document.querySelectorAll('.detenido-container').length,
                responsable: {
                    ...
                    presentacionPowerPoint: document.getElementById('presentacionPowerPoint').files[0] ? 
                                      document.getElementById('presentacionPowerPoint').files[0].name : ''
                }
            };
        }

        // ========== FUNCIÓN PARA RECOPILAR DATOS DE DETENIDOS ==========
        ...
        async function enviarDatosAGoogleSheets(datosCompletos) {
            ...
            try {
                ...
                console.log('  Variable seleccionada:', datosCompletos.formulario.variablesProductividad);
                console.log('  Valor de "Aeronaves (Und)":', datosCompletos.tablaVariables['Aeronaves (Und)']);
                console.log('  Todas las variables:', datosCompletos.tablaVariables);
                
                console.log('📤 Enviando datos completos:', datosCompletos);
                
                // 🔥 CORRECCIÓN: Definir formData correctamente
                const formData = new FormData();
                formData.append('tipo', 'operativo_policial_completo');
                formData.append('timestamp', datosCompletos.formulario.timestamp);
                ...
                formData.append('totalDetenidos', datosCompletos.totalDetenidos.toString());
                formData.append('tablaVariables', JSON.stringify(datosCompletos.tablaVariables));
                
                console.log('📦 FormData preparado con tablaVariables');
                console.log('  ¿Incluye "Aeronaves (Und)"?', datosCompletos.tablaVariables.hasOwnProperty('Aeronaves (Und)'));
                console.log('  Valor de "Aeronaves (Und)" en formData:', datosCompletos.tablaVariables['Aeronaves (Und)']);
                
                let response;
                let success = false;
                
                try {
                    console.log('🔄 Intentando POST con tablaVariables...');
                    response = await fetch(APPS_SCRIPT_URL_ENVIAR_DATOS, {
                        method: 'POST',
                        body: formData,
                        mode: 'no-cors'
                    });
                    ...
                    success = true;
                    console.log('✅ POST enviado (modo no-cors)');
                    
                } catch (postError) {
                    console.log('❌ POST falló:', postError);
                    
                    try {
                        console.log('🔄 Intentando GET como fallback...');
                        const params = new URLSearchParams();
                        params.append('tipo', 'operativo_policial_completo');
                        params.append('timestamp', datosCompletos.formulario.timestamp);
                        params.append('datosFormulario', JSON.stringify(datosCompletos.formulario));
                        params.append('detenidos', JSON.stringify(datosCompletos.detenidos));
                        params.append('totalDetenidos', datosCompletos.totalDetenidos.toString());
                        params.append('tablaVariables', JSON.stringify(datosCompletos.tablaVariables));
                        
                        response = await fetch(`${APPS_SCRIPT_URL_ENVIAR_DATOS}?${params.toString()}`, {
                            method: 'GET',
                            mode: 'no-cors'
                        });
                        
                        success = true;
                        console.log('✅ GET enviado (modo no-cors)');
                        
                    } catch (getError) {
                        console.log('❌ GET también falló:', getError);
                        success = false;
                    }
                }
                
                if (success) {
                    mostrarNotificacion('✅ Datos enviados exitosamente a Google Sheets', 'success', 5000);
                    console.log('✅ Datos enviados (confirmación limitada por CORS)');
                    console.log('✈️  Aeronaves (Und) enviado con valor:', datosCompletos.tablaVariables['Aeronaves (Und)']);
                    ...
                    setTimeout(() => {
                        ...
                        // Limpiar sección de variables
                        document.getElementById('variableSectionsContainer').innerHTML = '';
                        document.getElementById('validationSummary').style.display = 'none';
                        
                        mostrarNotificacion('✨ Formulario limpiado y listo para nuevo registro', 'info', 3000);
                    }, 2000);
                ...
                mostrarNotificacion(`❌ Error al enviar datos: ${error.message}. Verifica la conexión.`, 'error', 5000);
            } finally {
        ...
        // ========== NUEVO EVENT LISTENER DEL SUBMIT ==========
        document.getElementById('operativeForm').addEventListener('submit', async function(e) {
            e.preventDefault();
            
            console.log('🔄 Iniciando proceso de envío CORREGIDO...');
            ...
            if (!validarFormularioCompleto()) {
                mostrarNotificacion('❌ Error: Complete todos los campos requeridos del formulario', 'error', 5000);
                return;
            }
            
            // 🔥 USAR LA FUNCIÓN DE VERIFICACIÓN
            const datosCompletos = verificarEnvioDatos();
            
            // Mostrar confirmación antes de enviar
            if (confirm(`¿Está seguro de que desea enviar el formulario?\n\n✅ Variable seleccionada: ${datosCompletos.formulario.variablesProductividad}\n✅ Valor: ${datosCompletos.tablaVariables[datosCompletos.formulario.variablesProductividad]}\n✅ Se enviarán TODAS las variables de productividad`)) {
                try {
                    ...
                    mostrarNotificacion('❌ Error inesperado al procesar el formulario', 'error', 5000);
                }
            }
        });
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            console.log('🔗 URL Envío de datos:', APPS_SCRIPT_URL_ENVIAR_DATOS);
            ...
            console.log('📤 SISTEMA DE ENVÍO ACTUALIZADO: Se enviarán TODAS las variables de productividad como tabla');
            console.log('🔧 CORRECCIÓN APLICADA: Mapeo corregido de variables de productividad');
